...
//			level: "level",			//层级
	...
	DTree.prototype.setting = function(options) {
		...
		/** 菜单栏参数**/
		this.menubar = this.options.menubar || true;	//是否打开菜单栏
	...
	DTree.prototype.getDom = function(treeId, parentId, title, isLast, iconClass, checkArr, level, spread, disabled) {
		...
		return {
			...
			text: function() {	// 文字显示
				return "<cite class='"+LI_DIV_TEXT_CLASS+"' data-id='"+treeId+"' data-leaf='"+(isLast ? "leaf" : "node")+"' title='"+title+"'><div style='display: inline-block;height:25px;width:100px;margin-left:10px;margin-bottom:-5px;overflow: hidden; white-space: nowrap; text-overflow: ellipsis;'>"+title+"</div></cite>";
			},
			ul: function() {	//子节点ul
	...
	DTree.prototype.getMenubarDom = function(menu){
		...
		switch (menu) {
			case defaultMenu.moveDown:
				gg = "<button class='layui-btn layui-btn-sm layui-btn-primary' dtree-id='"+rootId+"' d-menu='"+defaultMenu.moveDown+"' title='展开节点'>展开</button>";
				break;
			case defaultMenu.moveUp:
				gg = "<button class='layui-btn layui-btn-sm layui-btn-primary' dtree-id='"+rootId+"' d-menu='"+defaultMenu.moveUp+"' title='收缩节点'>收缩</button>";
				break;
			case defaultMenu.refresh:
				gg = "<button class='layui-btn layui-btn-sm layui-btn-primary' dtree-id='"+rootId+"' d-menu='"+defaultMenu.refresh+"' title='刷新'>刷新</button>";
				break;
			...
			case defaultMenu.searchNode:
				gg = "<button class='layui-btn layui-btn-sm layui-btn-primary' dtree-id='"+rootId+"' d-menu='"+defaultMenu.searchNode+"' title='查询节点'>查询</button>";
				break;
		}
		return gg;
	};
	...
	DTree.prototype.menubarListener = function(menuId, flag){
		...
		switch (menuId) {
			...
			case defaultMenu.refresh:
				nodes = "";//先清空
				nodes = _this.checkbarNode;//刷新前获取全部子节点
	...
	DTree.prototype.getNowParam = function(){
		debugger
		var _this = this;
		qjbl = _this.getRequestParam(_this.getNodeParam());
