...
var Manage = {
	...
	title:"分类管理",
	elem: "layTable",	//表格id
    url: "/doc/classify/queryList"			//数据服务地址
};
...
Manage.initColumn = function () {
    return [ [
    	...
    	{title : '分类名称',field : 'classifyName',align : 'center'},
		{title : '上级分类名称',field : 'classifyParentName',align : 'center'},
    	{title : '创建时间',field : 'createDate',align : 'center',templet : "<div>{{layui.util.toDateString(d.createDate, 'yyyy-MM-dd HH:mm:ss')}}</div>"},
    	{title : '状态',field : 'status',align : 'center'},
    	{title : '排序',field : 'sort',align : 'center',sort : true}
    	] ];
};
    ...
    layui.use(['table','form'], function() {
		...
		table.on('toolbar('+Manage.elem+')', function(obj) {
			...
			case 'edit':
				...
				Manage.openDeptDetail(data[0].uuid);
				break;
			case 'delete':
				var data = checkStatus.data;
				if(data.length==0){
...
Manage.openAddDept = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/doc/classify/addClassify'
    });
...
Manage.openDeptDetail = function (id) {
    ...
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/doc/classify/editClassify/' + id
    });
    this.layerIndex = index;
...
Manage.delete = function (id) {
    ...
    $.ajax({
        type:"delete",
        url:Feng.ctxPath + "/doc/classify/delete",
        dataType:"json",
