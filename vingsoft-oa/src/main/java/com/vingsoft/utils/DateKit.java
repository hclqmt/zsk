...
public class DateKit {
     ...
     * @return
     */
    public static Date getDateByCondition(Date date, int days) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
     ...
     * @return
     */
    public static String getDiffHourMin(Date startDate, Date endDate) {
        long nd = 1000 * 24 * 60 * 60;//一天的毫秒数
        long nh = 1000 * 60 * 60;//一小时的毫秒数
        long nm = 1000 * 60;//一分钟的毫秒数
        long diff = endDate.getTime() - startDate.getTime();
        long minData = diff / 1000 / 60 / 60;
        if (minData < 1) {
            return "小于一分钟";
        }
        long hour2 = diff / nh;
        long min = diff % nd % nh / nm;//计算差多少分钟
        return hour2 + "小时" + min + "分钟";
    }
     ...
     * @return
     */
    public static long compareDateOfMin(Date date1, Date date2) {
        Calendar calendar1 = Calendar.getInstance();
        Calendar calendar2 = Calendar.getInstance();
        calendar1.setTime(date1);
        calendar2.setTime(date2);
    ...
    public static void main(String[] args) {
        System.out.println(DateKit.format(getDateByCondition(DateKit.nowOfDate(), 7), DateKit.YYYY_MM_DD_HH_MM_SS));
    }
}